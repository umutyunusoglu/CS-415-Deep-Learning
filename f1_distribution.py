...
CONFIG = {
    ...
    "processed_dir": "processed_test_slakh",  # Kendi processed test dataset yolunuz
...
def process_file_windows(model, mel_layer, filepath):
    ...
    try:
        ...
        return f1s, (
            num_windows - len(f1s)
        )  # Returns scores and count of silent windows dropped
...
def run_full_test():
    ...
    if all_active_f1_scores:
        ...
        plt.axvline(
            ...
            linestyle="--",
            linewidth=2,
            label=f"Mean: {mean_active_f1:.3f}",
        )
